...
#endif
{
    ...
    addParameter(freq = new AudioParameterFloat ("filterFreq", "Filter Freq", 20, 20000, 15000));
...
void OverdriveAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        filters[channel]->processSamples(channelData, N);
...
void OverdriveAudioProcessor::setFilterCoefficients()
{
    IIRCoefficients coeffs = IIRCoefficients::makeLowPass(fs, *freq);
